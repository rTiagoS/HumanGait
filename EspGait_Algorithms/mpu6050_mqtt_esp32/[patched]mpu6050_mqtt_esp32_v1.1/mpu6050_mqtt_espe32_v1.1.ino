...
unsigned long periodo_de_simulacao;
...
void datasend_interval()
{
  if (SIMULAR)
  {
    ...
    tempo_final = micros();
    periodo_de_simulacao = 5000000;
    while (((tempo_final - tempo_inicial) <= periodo_de_simulacao))
    {
      time_sample = (tempo_final - tempo_inicial);
      mpu_loop();
...
/*memset(buf, 0, sizeof(buf));
    sprintf(buf,"%d,%d,%d,%.2f,%.2f,%.2f,%d",aaReal.x,aaReal.y,aaReal.z,(ypr[0] * 180 / M_PI),(ypr[1] * 180 / M_PI),(ypr[2] * 180 / M_PI), time_sample);
    client.publish(mqtt_accel_csv, buf);
*/
void BigBuffer (char *GaitDataStream[], int NumeroDeAmostras)
{
  j= 0
  for (i = 0; i < sizeof(GaitDataStream)/sizeof(char); i++)
  GaitDataStream[i+0]=aaReal.x;
  GaitDataStream[i+1]=aaReal.y;
  GaitDataStream[i+2]=aaReal.z;
  GaitDataStream[i+3]=(ypr[0] * 180 / M_PI);
  GaitDataStream[i+4]=(ypr[1] * 180 / M_PI);
  GaitDataStream[i+5]=(ypr[2] * 180 / M_PI);
  j+=6;
}
